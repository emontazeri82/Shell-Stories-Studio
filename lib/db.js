...
// ⚠️ RAW DB RESULT — DO NOT USE IN UI
...
export async function getProductWithMediaById(id) {
  ...
  try {
    ...
    return {
      ...
      is_favorite: Number(product.is_favorite || 0),

      // 🖼️ BASIC PRODUCT FIELDS
...
export async function getAllProductsForList() {
  ...
  try {
    ...
    return rows.map((r) => ({
      ...
      discount_active: Number(r.discount_active || 0),
