...
export async function updateProductById(id, productData) {
  ...
  try {
    const result = await db.run(
      `UPDATE products
       SET name = ?, description = ?, price = ?, stock = ?, image_url = ?, category = ?, is_active = ?
       WHERE id = ?`,
      [name, description, price, stock, image_url, category, is_active, numericId]
    );

    if (result.changes === 0) {
      throw new Error('No product updated (ID not found)');
    }
