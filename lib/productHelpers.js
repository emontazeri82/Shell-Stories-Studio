...
export async function updateProductById(id, patch) {
  ...
  // Keep only allowed + defined fields
  ...
  // 🔁 Coercion — SAME behavior as before, just explicit
  const coerced = entries.map(([key, value]) => {
  ...
  const columns = coerced.map(([key]) => `${key} = ?`).join(", ");
  const values = coerced.map(([, value]) => value);
  ...
  try {
    ...
    console.log(`🧩 Updated product id=${id} → ${columns}`);
    return result;
  } catch (err) {
