...
export default function PriceBreakdown({ items = [], showButtons = false, onCheckout, onClear }) {
    ...
    return (
      <div className="mt-6 space-y-3">
        ...
        <div className="flex justify-between text-sm text-gray-600 dark:text-gray-300">
          <span>Estimated Shipping</span><span>{shipping === 0 ? "Free" : `$${shipping.toFixed(2)}`}</span>
        </div>
        ...
        {showButtons && (
          <>
            <button onClick={onCheckout} className="w-full bg-indigo-600 text-white py-2 rounded hover:bg-indigo-700 transition">
              Proceed to Checkout
            </button>
            <button onClick={onClear} className="w-full text-sm text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
              Clear Cart
            </button>
          </>
        )}
